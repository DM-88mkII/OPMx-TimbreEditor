...
,m_aaParam{
	{	IDC_TIMBRE_EN_STATIC,
		...
		IDC_TIMBRE_FRQ_STATIC,
		IDC_TIMBRE_AMS_STATIC,
		IDC_TIMBRE_AMD_STATIC,
		IDC_TIMBRE_PMS_STATIC,
		IDC_TIMBRE_PMD_STATIC,
		IDC_TIMBRE_LFR_STATIC,
	...
	switch (pMsg->message){
		case WM_KEYDOWN:{
			...
			if (bControl){
				...
				switch (pMsg->wParam){
					case VK_HOME:{		m_Octave += (m_Octave < 8)? 1: 0;	Log(_T("o{}"), m_Octave);	return TRUE;	}
					case VK_END:{		m_Octave -= (m_Octave > 1)? 1: 0;	Log(_T("o{}"), m_Octave);	return TRUE;	}
...
void CModuleTab::OnBnClickedModuleLoadButton()
{
	...
	CFileDialog FileDialog(true, _T("opmx"), _T("*.opmx"), OFN_FILEMUSTEXIST | OFN_EXTENSIONDIFFERENT);
...
void CModuleTab::OnBnClickedModuleSaveButton()
{
	...
	CFileDialog FileDialog(false, _T("opmx"), _T("*.opmx"), OFN_OVERWRITEPROMPT | OFN_EXTENSIONDIFFERENT);
	if (FileDialog.DoModal() == IDOK){
		SetTabName(FileDialog.GetFileTitle());
